  ...
  const result = await Promise.all(
    majorDocs.map(async (major) => {
      ...
      applications.forEach((app) => {
        if (app.pnp === 'PASS') {
          avgGPA += app.applyGPA;
          minGPA = Math.min(minGPA, app.applyGPA);
          passCount++;
        }
      });

      let displayName = `${major.collegeName} ${major.name}`;

      return {
        name: displayName, 
        majorEngShort: major.shortEngName,
        collegeEngShort: major.shortCollegeEngName,
        applyNum: applications.length,
        passNum: passCount,
        avg: passCount ? avgGPA / passCount : 0,
        min: minGPA === 4.5 ? 0 : minGPA,
  ...
  try {
    ...
    for (let i = 0; i < currentMetadata.length; i++) {
      ...
      const returndata = {
        ...
        recruitNumber: metadata.expectedRecruitNumber,
        applyNumber: metadata.appliedNumber,
        competition: metadata.expectedRecruitNumber
          ? Number(
              (metadata.appliedNumber / metadata.expectedRecruitNumber).toFixed(
                2,
              ),
            )
          : 0,
